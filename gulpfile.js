...
function copyTask() {
  return gulp
    .src(['src/*.js', 'src/icons/**/*.{svg,png}'], { base: 'src' })
    .pipe(gulp.dest('build'));
}

function copyImgTask() {
  return gulp.src('src/img/**/*.{svg,png,jpg}').pipe(gulp.dest('build/img'));
}
...
function serveTask() {
  ...
  gulp.watch('src/*.js', copyTask).on('change', browserSync.reload);
...
async function publishTask() {
  await ghpages.publish(
    'build',
    { branch: 'gh-pages', repo: 'https://github.com/EjikBezN0jek/toxin.git' },
    function (err) {}
  );
}
